...
from model import HyperGCNBranch
...
def PredictScore(train_drug_dis_matrix, drug_matrix, dis_matrix, herb_chemical,seed, epochs, emb_dim, dp, lr, adjdp):
    ...
    X1=csr_matrix(herb_chemical)# [num_herbs, num_chemical]
    ...
    model_hyper=HyperGCNBranch(input_dim=features_hyper[2][1], emb_dim=emb_dim, adj=A_hyper_adj)
    ...
    for epoch in range(epochs):
        with tf.GradientTape() as tape:
            ...
            # loss_value = norm*tf.reduce_mean(bce_loss)
        ...
        if epoch % 500 == 0:
            print(f"Epoch {epoch:04d} | Loss: {loss_value.numpy():.5f} | Accuracy: {current_accuracy:.5f}")
    ...
    drug_disease_res = model(inputs_data, training=False)
