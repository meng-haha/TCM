...
def PredictScore(train_drug_dis_matrix, drug_matrix, dis_matrix, herb_chemical,seed, epochs, emb_dim, dp, lr, adjdp):
    ...
    A_hyper_adj = pad_matrix(A_hyper_adj, (1444, 1444))# 填充为维度为2033的矩阵
    A_hyper_adj=tf.sparse.from_dense(tf.cast(A_hyper_adj,tf.float32))#转为稀疏矩阵
    X1=pad_matrix_bottom(herb_chemical,1444) #填充为行为2033的矩阵
...
if __name__=="__main__":
    ...
    simw = 6
    seed=42
    dis_sim = pd.read_excel('../Ldata/disease_similarity.xlsx', engine='openpyxl', header=None).values
    drug_sim = pd.read_excel('../Ldata/LRSSL_similarity_matrix.xlsx', engine='openpyxl', header=None).values

    drug_dis_matrix = pd.read_excel('../Ldata/drug_dis_mat.xlsx', engine='openpyxl', header=None).values
    herb_chemical = pd.read_excel('../Ldata/drug_pubchem_mat.xlsx', engine='openpyxl', header=None).values

    # dis_sim = pd.read_csv('../Ldata/disease_similarity.txt', delim_whitespace=True)
    # drug_dis_matrix = pd.read_csv('../Ldata/drug_dis_mat.txt', delim_whitespace=True)
    # herb_chemical=pd.read_csv('../Ldata/drug_pubchem_mat.txt', delim_whitespace=True)
